...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        debug!("task0.time: {}", task0.task_time);
    ...
    fn mark_current_exited(&self) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].task_time = 0;
        inner.tasks[current].task_status = TaskStatus::Exited;
        inner.tasks[current].task_syscall_times = [0; MAX_SYSCALL_NUM];
    }
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            inner.tasks[current].task_time = get_time_ms() - time;
    ...
    /// Follow fucntion for the syscall  `task_info()`
...
/// PLUS 1 for the current task and the syscall id
